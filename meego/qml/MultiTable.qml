/*
    Table for three- and four-player Schnapsen, MeeGo Harmattan edition: the
    QtQuick 1.1 port of qml-common/MultiTable.qml. Differences: variant
    instead of var properties, no read-only properties, a Binding element
    for the right seat and a sourceComponent toggle for the top seat.
...
Item {
    ...
    // Landscape: the opponents sit at the sides of the trick, everything
    // else shrinks a little (k).
    property bool landscape: width > height
    property real k: landscape ? 0.72 : 1
    ...
    function handCardWidth(count) {
        var maxWidth = Style.itemSizeLarge * 1.02 * k
    ...
    Component {
        ...
        Item {
            id: seatItem
            property int seat: 1
            property variant info: seat < table.seatList.length ? table.seatList[seat] : ({})
            width: table.landscape ? table.width * 0.22 : table.width * (table.players === 3 ? 0.44 : 0.31)
            ...
            Column {
                ...
                Item {
                    id: seatCards
                    width: parent.width
                    height: Style.itemSizeSmall * 0.62 * table.k
                    ...
                    Repeater {
                        ...
                        Card {
                            ...
                            width: Style.itemSizeSmall * 0.38 * table.k
                            height: width * 1.4
                            x: (seatCards.width - width - (count - 1) * width * 0.34) / 2 + index * width * 0.34
                            cardId: "0_10"
                            cardStyle: table.cardStyle
                            faceUp: false
                        }
    ...
    Loader {
        ...
        anchors.top: header.bottom
        anchors.topMargin: Style.paddingSmall
        sourceComponent: table.players === 4 ? seatComponent : undefined
    ...
    Binding {
        target: rightSeat.item
        property: "seat"
        value: table.players === 4 ? 3 : 2
    }
    ...
    Item {
        ...
        anchors.topMargin: table.landscape ? Style.paddingSmall : Style.paddingMedium
        anchors.bottom: infoRow.top
        anchors.bottomMargin: Style.paddingSmall
        width: table.landscape ? parent.width * 0.5 : parent.width * 0.86

        property real cardHeight: Math.min(Style.itemSizeLarge * 1.2 * table.k, height * 0.46)

        Repeater {
            model: engine.trick
            Card {
                property variant slot: table.trickSlot(modelData.seat)
    ...
    Item {
        ...
        anchors.left: parent.left
        anchors.right: parent.right
        height: Math.max(decisionFlow.height, Style.itemSizeSmall * 0.8 * table.k)
    ...
    Item {
        ...
        anchors.left: parent.left
        anchors.right: parent.right
        anchors.leftMargin: table.landscape ? parent.width * 0.2 : 0
        anchors.rightMargin: table.landscape ? parent.width * 0.2 : 0
        anchors.bottom: parent.bottom
        anchors.bottomMargin: Style.paddingSmall
        height: Style.itemSizeLarge * 1.5 * table.k
    ...
    // Landscape: the side seats flank the trick instead of sitting above it.
    states: State {
        name: "landscape"
        when: table.landscape
        AnchorChanges {
            target: trickArea
            anchors.top: table.players === 4 ? topSeat.bottom : header.bottom
        }
        AnchorChanges {
            target: leftSeat
            anchors.top: undefined
            anchors.verticalCenter: trickArea.verticalCenter
        }
        AnchorChanges {
            target: rightSeat
            anchors.top: undefined
